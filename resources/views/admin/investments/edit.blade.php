...
<div class="admin-posts-container">
    ...
    <div class="admin-posts-header">
        <h1 class="admin-title"><i class="fas fa-edit"></i> تعديل فرصة استثمارية</h1>
        <p class="admin-subtitle">قم بتعديل بيانات الفرصة الاستثمارية الحالية</p>
    </div>
    ...
    <div class="admin-card">
        <div class="admin-card-header">
            ...
            <div class="admin-card-actions">
                <div class="d-flex gap-2">
                    <a href="{{ route('investments.show', $investment->slug) }}" class="admin-btn admin-btn-outline" target="_blank">
                        <i class="fas fa-eye"></i> عرض الفرصة
                    </a>
        ...
        <div class="admin-card-body">
            ...
            <form action="{{ route('admin.investments.update', $investment) }}" method="POST" enctype="multipart/form-data" id="investment-form">
                @csrf
                @method('PUT')
                ...
                <div class="row">
                    ...
                    <div class="col-lg-8">
                        <div class="form-group">
                            ...
                            <input type="text" id="title" name="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title', $investment->title) }}" required>
                            @error('title')
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="content" id="content" class="hidden @error('content') is-invalid @enderror">{{ old('content', $investment->content) }}</textarea>
                            @error('content')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="admin-card mb-4">
                            ...
                            <div class="admin-card-body">
                                <div class="form-group">
                                    ...
                                    <div class="mt-3" id="featured-image-preview">
                                        @if($investment->featured_image)
                                        <img src="{{ asset('storage/' . $investment->featured_image) }}" class="img-fluid rounded mt-2" style="max-height: 200px;" alt="{{ $investment->title }}">
                                        @endif
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="admin-card mb-4">
                            ...
                            <div class="admin-card-body">
                                <div class="form-group">
                                    ...
                                    <textarea id="investment_highlights" name="investment_highlights" class="form-control @error('investment_highlights') is-invalid @enderror" rows="4" placeholder="أدخل المميزات مفصولة بفواصل">{{ old('investment_highlights', is_array($investment->investment_highlights) ? implode('، ', $investment->investment_highlights) : (is_string($investment->investment_highlights) ? implode('، ', json_decode($investment->investment_highlights) ?: []) : '')) }}</textarea>
                        ...
                        <div class="admin-card-body">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <input type="text" id="investment_amount" name="investment_amount" class="form-control @error('investment_amount') is-invalid @enderror" value="{{ old('investment_amount', $investment->investment_amount) }}" required>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control @error('investment_category_id') is-invalid @enderror" id="investment_category_id" name="investment_category_id" required>
                                    ...
                                    <option value="{{ $category->id }}" {{ old('investment_category_id', $investment->investment_category_id) == $category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group mt-3">
                                ...
                                <input type="text" id="location" name="location" class="form-control @error('location') is-invalid @enderror" value="{{ old('location', $investment->location) }}" placeholder="مثل: الرياض، جدة، الدمام، إلخ" required>
                                @error('location')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            @php
                                            $tags = is_array($investment->tags) ?
                                            implode(", ", $investment->tags) :
                                            (is_string($investment->tags) ?
                                            implode(", ", json_decode($investment->tags) ?: []) :
                                            '');
                                            @endphp
                                            <input type="text" id="tags" name="tags" class="form-control" value="{{ old('tags', $tags) }}" placeholder="أدخل الوسوم مفصولة بفواصل">
                                            <small class="text-muted">أدخل الوسوم مفصولة بفواصل (مثال: استثمار، عقارات، تجارة)</small>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label for="contact_info" class="form-label">معلومات التواصل</label>
                                            <textarea id="contact_info" name="contact_info" class="form-control @error('contact_info') is-invalid @enderror" rows="4" placeholder="معلومات التواصل للمستثمرين المهتمين">{{ old('contact_info', $investment->contact_info) }}</textarea>
                                            @error('contact_info')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                    ...
                    <div class="col-12">
                        <div class="admin-card mb-4">
                            ...
                            <div class="admin-card-body">
                                ...
                                <div class="mb-4">
                                    <h4 class="mb-3">الصور الحالية</h4>
                                    <div class="gallery-preview">
                                        @foreach($investment->images as $index => $image)
                                        <div class="gallery-item {{ in_array($image->id, old('images_to_delete', [])) ? 'marked-for-delete' : '' }}">
                                            ...
                                            <button type="button" class="gallery-remove delete-image-btn" data-image-id="{{ $image->id }}">
                                                <i class="fas fa-times"></i>
                                            </button>
                                            <input type="hidden" name="existing_image_ids[]" value="{{ $image->id }}">
                                            <input type="checkbox" name="images_to_delete[]" value="{{ $image->id }}" class="delete-image-checkbox hidden" {{ in_array($image->id, old('images_to_delete', [])) ? 'checked' : '' }}>
                                        </div>
                                        <div class="gallery-caption">
                                            <input type="text" name="existing_image_captions[]" class="form-control form-control-sm" value="{{ old('existing_image_captions.' . $index, $image->caption) }}" placeholder="وصف الصورة">
                                        </div>
                                        @endforeach
                                    </div>
                                </div>
                                ...
                                <!-- Add New Images -->
                                <div class="form-group">
                                    <label for="gallery_images" class="form-label">إضافة صور جديدة للمعرض</label>
                        ...
                        <div class="admin-card-body">
                            <div class="form-group">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="is_published" name="is_published" value="1" {{ old('is_published', $investment->is_published) ? 'checked' : '' }}>
                            ...
                            <div class="form-group mt-3">
                                ...
                                <input type="text" id="published_at" name="published_at" class="form-control flatpickr" value="{{ old('published_at', $investment->published_at ? $investment->published_at->format('Y-m-d') : '') }}" placeholder="اختر تاريخ النشر">
                            </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        deleteImageBtns.forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                const checkbox = document.querySelector(`input[name="images_to_delete[]"][value="${imageId}"]`);
                ...
                if (checkbox) {
                    ...
                    if (!isChecked) {
                        galleryItem.classList.add('marked-for-delete');
                        this.innerHTML = '<i class="fas fa-undo"></i>';
                    } else {
                        galleryItem.classList.remove('marked-for-delete');
        ...
        // Handle form cancel
        const cancelBtn = document.querySelector('.admin-btn-outline:last-child');

        cancelBtn.addEventListener('click', function(e) {
            e.preventDefault();

            if (confirm('هل أنت متأكد من إلغاء تعديل الفرصة الاستثمارية؟ سيتم فقدان جميع التغييرات.')) {
